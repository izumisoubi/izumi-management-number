...
function initSourceGuide(){
  ...
  let savedGuide={};
  try{savedGuide=JSON.parse(localStorage.getItem('izumi_sales_demo_mirror_ledgerSourceGuide')||'{}')}catch(_error){savedGuide={}}
  ...
  handle.addEventListener('pointerup',()=>{
    drag=null;
    const rect=guide.getBoundingClientRect();
    localStorage.setItem('izumi_sales_demo_mirror_ledgerSourceGuide',JSON.stringify({left:rect.left,top:rect.top,collapsed:guide.classList.contains('collapsed')}));
  });
}
...
window.addEventListener('storage',event=>{if(event.key==='izumi_sales_demo_mirror_-ledger-refresh')scheduleLedgerReload();});
document.addEventListener('visibilitychange',()=>{
